...
## [2026-05-07] Video transcript workflows must be captions-first and single-pass

**What failed:** The first pass for YouTube transcript + takeaways + PDF packaging used a heavier, multi-step path (download audio, transcribe, multiple parse retries) without a clean deterministic pipeline, which increased tool calls and made traceability less crisp than needed.
**Workaround:** Standardize to a captions-first, fallback-to-ASR pipeline: (1) fetch metadata + subtitles first, (2) only transcribe audio when captions are missing/incomplete, (3) parse cues once into a normalized structure reused for transcript and timestamped takeaways, (4) generate markdown outputs from that single source, (5) render both PDFs, (6) run final file verification and emit compact tool audit table.
**Hit count:** 1
**Status:** resolved